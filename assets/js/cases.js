...
/* ═════════════════════════════════════════════════════════════════════════
   01 — CONSULTING PERFORMANCE (POWER BI)
   ═════════════════════════════════════════════════════════════════════════ */
{
  slug: "consulting-utilization",
  num: "01",
  title: "Consulting Performance",
  family: T("Pilotage BI", "BI reporting"),
  familyKey: "bi",
  accent: "#1F4FD8",
  accentDark: "#8FA8FF",
  tags: ["Power BI", "DAX", "Power Query (M)", "TMDL", "Star schema", "Python"],
  cover: "assets/img/shots/consulting-utilization/thumbs/01-direction.webp",

  headline: T(
    "Un jour de consultant non vendu est perdu pour toujours : 4 389 jours d'intercontrat ont coûté 1,51 M€, et 44 missions sur 94 ont dépassé leur budget — sans qu'aucune ligne de la comptabilité ne le montre.",
    "An unsold consultant day is lost for good: 4,389 bench days cost €1.51M, and 44 engagements out of 94 overran their budget — without a single accounting line showing it."),

  pitch: T(
    "Un cabinet de conseil vend du temps. Le rapport Power BI transforme les feuilles de temps en système d'alerte : quatre pages, une par décideur, du taux d'occupation du cabinet jusqu'à la ligne de saisie à vérifier avant la clôture.",
    "A consulting firm sells time. The Power BI report turns timesheets into an early-warning system: four pages, one per decision-maker, from the firm-wide utilization rate down to the timesheet line to check before month-end close."),

  problem: T(
    "Dans une ESN ou un cabinet de conseil, le seul stock est le jour-homme — et un jour non vendu ne se rattrape jamais. Trois fuites de marge échappent pourtant à la comptabilité : l'intercontrat (des consultants payés mais non staffés), les forfaits qui consomment plus de jours que vendus, et l'érosion du TJM réellement encaissé. Les feuilles de temps contiennent toute l'information, mais elles ne servent qu'à facturer : le directeur financier découvre fin janvier que décembre était mauvais, quand les jours perdus ne peuvent plus être staffés.",
    "In an IT services or consulting firm, the only inventory is the person-day — and an unsold day can never be recovered. Yet three margin leaks escape accounting entirely: bench time (consultants paid but not staffed), fixed-price engagements consuming more days than were sold, and erosion of the day rate actually collected. Timesheets hold all of that information, but they are only used for invoicing: the CFO finds out at the end of January that December was bad, when the lost days can no longer be staffed."),

  dataset: {
    source: T("Généré par scripts/generate_dataset.py, graine fixe. Toutes les données sont synthétiques — noms de consultants, de clients et de missions inventés. Une seconde version « brute » simule six exports sales (feuilles de temps au format large, export SIRH Excel, CRM en JSON imbriqué, CSV en cp1252, facturation avec ligne TOTAL) pour pratiquer le nettoyage en Power Query.",
              "Generated by scripts/generate_dataset.py with a fixed seed. All data is synthetic — consultant, client and engagement names are made up. A second “raw” version simulates six messy exports (wide-format timesheets, an Excel HR extract, CRM as nested JSON, a cp1252 CSV, invoicing with a TOTAL row) to practise cleaning in Power Query."),
    volume: T("20 mois (janvier 2025 → août 2026) : 120 consultants, 5 practices, 30 clients, 96 missions, 64 096 lignes de feuilles de temps, 584 factures mensuelles, 100 objectifs grade × mois.",
              "20 months (January 2025 → August 2026): 120 consultants, 5 practices, 30 clients, 96 engagements, 64,096 timesheet rows, 584 monthly invoices, 100 grade × month targets."),
    fields: ["Fact_Temps", "Fact_Facturation", "Fact_Objectifs", "Dim_Date", "Dim_Consultant",
             "Dim_Grade", "Dim_Practice", "Dim_Mission", "Dim_Client", "Dim_Activite", "Dim_Pont_Marge"],
    note: T("Le staffing est simulé par périodes d'affectation (une mission, une allocation de 40 à 100 %, une durée), modulées par la tension de marché de chaque practice et une saisonnalité réaliste (creux d'août, pic d'automne). Le budget vendu de chaque mission est ensuite recalibré sur sa consommation réelle, avec une loi d'avancement propre au type de contrat : le forfait dérape plus souvent que la régie. Quelques défauts sont injectés volontairement — saisies non validées, factures en litige, journées déclarées à 1,25 jour.",
              "Staffing is simulated as assignment periods (one engagement, a 40 to 100% allocation, a duration), modulated by each practice's market tension and realistic seasonality (the August trough, the autumn peak). Each engagement's sold budget is then recalibrated on its actual consumption, with a progress distribution specific to the contract type: fixed-price overruns more often than time-and-materials. A few defects are injected on purpose — unvalidated entries, disputed invoices, days logged as 1.25 days.")
  },

  links: {},

  context: {
    secteur: T("Conseil et ESN — Deloitte, Atos, Capgemini, mais aussi toute PME de services (agence, bureau d'études, cabinet comptable)",
               "Consulting and IT services — Deloitte, Atos, Capgemini, but also any services SME (agency, engineering firm, accounting practice)"),
    modele: T("Vente de jours-homme en régie, au forfait ou en TMA ; 5 practices (Data & AI, Cloud, Cyber, ERP, Strategy), 5 grades de Junior à Director",
              "Selling person-days on time-and-materials, fixed-price or managed-services contracts; 5 practices (Data & AI, Cloud, Cyber, ERP, Strategy), 5 grades from Junior to Director"),
    role: T("Analyste BI de bout en bout : cadrage métier, génération des données, nettoyage Power Query, modélisation, DAX, conception UX des quatre pages",
            "End-to-end BI analyst: business framing, data generation, Power Query cleaning, modelling, DAX, UX design of the four pages"),
    perimetre: T("20 mois, 120 consultants, 96 missions, 64 096 lignes de temps ; 4 pages, chacune dédiée à un décideur",
                 "20 months, 120 consultants, 96 engagements, 64,096 timesheet rows; 4 pages, each dedicated to one decision-maker"),
    calcul: T("Modèle en étoile de 11 tables et 14 relations, ~55 mesures DAX ; rafraîchissement complet en quelques secondes",
              "Star schema of 11 tables and 14 relationships, ~55 DAX measures; full refresh in a few seconds"),
    stack: "Power BI Desktop · DAX · Power Query (M) · TMDL / PBIP · Python · pandas"
  },

  kpis: [
    { value: 68.6, label: T("Taux d'occupation", "Utilization rate"),
      note: T("pour une cible de 80 % · 26 960 jours facturables sur 39 327 disponibles", "against an 80% target · 26,960 billable days out of 39,327 available"),
      suffix: " %", decimals: 1 },
    { value: 1.51, label: T("Coût de l'intercontrat", "Bench cost"),
      note: T("4 389 jours payés et non vendus sur 20 mois", "4,389 days paid and not sold over 20 months"),
      suffix: T(" M€", "M€"), decimals: 2 },
    { value: 44, label: T("Missions en dépassement", "Overrunning engagements"),
      note: T("sur 94 · 943 jours hors budget · 342 k€ de marge perdue", "out of 94 · 943 days over budget · €342k of margin lost"),
      suffix: "", decimals: 0 },
    { value: 382, label: T("Anomalies de saisie", "Timesheet anomalies"),
      note: T("1 902 jours non validés · 49 factures en litige", "1,902 unvalidated days · 49 disputed invoices"),
      suffix: "", decimals: 0 }
  ],

  questions: [
    T("Le cabinet vend-il assez de ses jours disponibles, et si non, est-ce un problème de practice, de grade ou de saison ?",
      "Is the firm selling enough of its available days — and if not, is it a practice problem, a grade problem or a seasonal one?"),
    T("Quelles missions consomment plus de jours que vendus, et combien de marge cela détruit-il avant la fin du contrat ?",
      "Which engagements consume more days than were sold, and how much margin does that destroy before the contract ends?"),
    T("Les chiffres présentés en comité sont-ils fiables : combien de saisies sont encore en attente, combien de factures en litige ?",
      "Are the figures shown in committee reliable: how many entries are still pending, how many invoices are disputed?")
  ],

  framework: {
    intro: T(
      "Un cabinet se pilote avec un seul KPI central — le taux d'occupation — mais il ne dit rien s'il n'est pas décomposé. Le cadrage a consisté à relier ce taux à la marge, puis à la mission, puis à la ligne de saisie, avec une page par personne qui décide.",
      "A firm is steered with a single central KPI — the utilization rate — but it says nothing unless it is broken down. The framing was about linking that rate to margin, then to the engagement, then to the timesheet line, with one page per person who decides."),
    metrics: [
      { name: T("Taux d'occupation", "Utilization rate"),
        why: T("jours facturables ÷ jours disponibles — congés et maladie exclus du dénominateur ; la cible est une moyenne pondérée par grade, pas une moyenne simple",
               "billable days ÷ available days — leave and sickness excluded from the denominator; the target is a grade-weighted average, not a plain mean") },
      { name: T("Coût du dépassement", "Overrun cost"),
        why: T("jours consommés au-delà du budget vendu × coût journalier propre à chaque mission",
               "days consumed beyond the sold budget × each engagement's own daily cost") },
      { name: T("Coût de l'intercontrat", "Bench cost"),
        why: T("le chiffre qui fait réagir un comité : ce que coûte le temps qu'on ne vend pas",
               "the number that makes a committee react: what unsold time costs") }
    ],
    dimensions: [
      { name: T("Practice", "Practice"),
        why: T("chaque pôle a sa tension de marché et son directeur — l'écart entre practices est le premier levier de réallocation",
               "each practice has its own market tension and director — the gap between practices is the first reallocation lever") },
      { name: T("Grade", "Grade"),
        why: T("la cible dépend du grade (85 % pour un Junior, 50 % pour un Director) : sans cette dimension, le trou d'occupation se cache",
               "the target depends on grade (85% for a Junior, 50% for a Director): without this dimension, the utilization gap hides") },
      { name: T("Type de contrat", "Contract type"),
        why: T("le risque de marge est porté par le forfait, pas par la régie — il faut les séparer pour le voir",
               "margin risk is carried by fixed-price, not by time-and-materials — they must be separated to see it") }
    ]
  },

  method: [
    T("6 exports sales", "6 messy exports"),
    T("Power Query (M)", "Power Query (M)"),
    T("Étoile · 14 relations", "Star · 14 relationships"),
    T("~55 mesures DAX", "~55 DAX measures"),
    T("4 pages · 4 décideurs", "4 pages · 4 decision-makers")
  ],

  figures: [
    {
      img: "assets/img/shots/consulting-utilization/01-direction.webp",
      thumb: "assets/img/shots/consulting-utilization/thumbs/01-direction.webp",
      title: T("Page Direction : lisible en trente secondes — l'occupation est sous la cible, et le pont de marge montre où part l'argent",
               "Overview page: readable in thirty seconds — utilization is below target, and the margin bridge shows where the money goes"),
      read: T("Cinq cartes KPI comparées à leur cible, occupation mensuelle en barres + courbe avec la ligne des 80 %, occupation par practice triée, pont du CA facturé à la marge brute (production, intercontrat, avant-vente, interne) et top clients en marge directe. La période est glissante sur 12 mois.",
              "Five KPI cards compared with their target, monthly utilization as bars + line with the 80% line, utilization by practice sorted, a bridge from billed revenue to gross margin (delivery, bench, pre-sales, internal) and top clients by direct margin. The period rolls over 12 months.")
    },
    {
      img: "assets/img/shots/consulting-utilization/02-occupation-staffing.webp",
      thumb: "assets/img/shots/consulting-utilization/thumbs/02-occupation-staffing.webp",
      title: T("La carte de chaleur du staffing fait apparaître l'été de Strategy & Ops à 16 % puis 0 % — et le graphique par grade révèle que ce sont les Managers, pas les Juniors, qui manquent leur cible",
               "The staffing heatmap exposes Strategy & Ops' summer at 16% then 0% — and the by-grade chart reveals it is Managers, not Juniors, who miss their target"),
      read: T("Matrice practice → grade → consultant × mois avec échelle de couleur, dépliable jusqu'au consultant. Palmarès des jours d'intercontrat, répartition du temps en 100 % empilé (production, avant-vente, intercontrat, congés), occupation réelle contre cible par grade.",
              "A practice → grade → consultant × month matrix with a colour scale, drillable down to the consultant. A ranking of bench days, time allocation as 100% stacked columns (delivery, pre-sales, bench, leave), actual utilization against target by grade.")
    },
    {
      img: "assets/img/shots/consulting-utilization/03-rentabilite-missions.webp",
      thumb: "assets/img/shots/consulting-utilization/thumbs/03-rentabilite-missions.webp",
      title: T("La matrice de risque isole le quadrant à traiter : les losanges rouges (forfaits) passé 100 % d'avancement et sous 25 % de marge",
               "The risk matrix isolates the quadrant to act on: red diamonds (fixed-price) past 100% progress and under 25% margin"),
      read: T("Une bulle par mission : avancement en abscisse, taux de marge en ordonnée, taille = montant vendu, forme et couleur = type de contrat. Lignes de référence à 100 % et 25 %. À droite, les missions en dépassement triées par coût, jours vendus contre jours consommés.",
              "One bubble per engagement: progress on the x-axis, margin rate on the y-axis, size = amount sold, shape and colour = contract type. Reference lines at 100% and 25%. On the right, overrunning engagements sorted by cost, days sold against days consumed.")
    },
    {
      img: "assets/img/shots/consulting-utilization/04-qualite-donnees.webp",
      thumb: "assets/img/shots/consulting-utilization/thumbs/04-qualite-donnees.webp",
      title: T("La page que personne ne demande et qui fait adopter le rapport : le contrôleur de gestion vérifie les chiffres avant de les présenter",
               "The page nobody asks for and that gets the report adopted: the financial controller checks the numbers before presenting them"),
      read: T("Saisies non validées, taux de complétude, factures en litige et lignes anormales (plus d'un jour déclaré sur une ligne). Le journal des temps et le tableau des anomalies restent exportables, avec le statut coloré ligne à ligne.",
              "Unvalidated entries, completeness rate, disputed invoices and abnormal rows (more than one day logged on a single line). The timesheet log and the anomaly table stay exportable, with the status colour-coded line by line.")
    }
  ],

  insights: [
    {
      title: T("Une practice décroche de vingt points, et un taux global la noie",
               "One practice falls twenty points behind, and a firm-wide rate drowns it"),
      data: T("Occupation de 68,9 % à 72,7 % pour quatre practices sur cinq ; Strategy & Ops tombe à 49,2 %, avec un été à 16 % puis 0 %. Le taux du cabinet, 68,6 %, reste en apparence proche des autres.",
              "Utilization between 68.9% and 72.7% for four practices out of five; Strategy & Ops drops to 49.2%, with a summer at 16% then 0%. The firm-wide rate, 68.6%, looks close to the others."),
      soWhat: T("Le problème n'est pas commercial pour tout le cabinet : il est concentré. Une réallocation temporaire de consultants Strategy vers les practices en tension rapporte plus vite qu'une campagne commerciale générale.",
                "The problem is not commercial for the whole firm: it is concentrated. Temporarily reallocating Strategy consultants to practices under pressure pays back faster than a general sales push."),
      driver: T("La tension de marché la plus faible des cinq pôles, amplifiée par le creux d'août.",
                "The weakest market tension of the five practices, amplified by the August trough."),
      team: T("Direction · Directeurs de practice", "Leadership · Practice directors")
    },
    {
      title: T("Le trou d'occupation vient de l'encadrement, pas des juniors",
               "The utilization gap comes from management, not from juniors"),
      data: T("Les Managers sont à 49,9 % pour une cible de 70 % ; les Juniors à 70,4 % pour 85 %, les Directors au-dessus de la leur (53,5 % pour 50 %).",
              "Managers sit at 49.9% against a 70% target; Juniors at 70.4% against 85%, Directors above theirs (53.5% against 50%)."),
      soWhat: T("C'est contre-intuitif, donc précieux : le réflexe est de chercher à staffer les juniors. Or l'écart le plus coûteux est chez les profils les plus chers, qui passent leur temps en interne et en avant-vente.",
                "It is counter-intuitive, hence valuable: the reflex is to staff juniors. Yet the costliest gap sits with the most expensive profiles, who spend their time on internal work and pre-sales."),
      driver: T("Une cible pondérée par grade — une moyenne simple des cibles aurait masqué l'écart.",
                "A grade-weighted target — a plain average of targets would have hidden the gap."),
      team: T("RH · Staffing", "HR · Staffing")
    },
    {
      title: T("Le forfait porte le risque de marge : 62 % dérapent, contre 33 % en régie",
               "Fixed-price carries the margin risk: 62% overrun, against 33% on time-and-materials"),
      data: T("44 missions sur 94 dépassent leur budget en jours, pour 943 jours hors budget et 342 k€ de marge perdue. La part des missions en dépassement monte à 62 % au forfait, 40 % en TMA, 33 % en régie.",
              "44 engagements out of 94 exceed their budget in days, for 943 days over budget and €342k of margin lost. The share of overrunning engagements reaches 62% on fixed-price, 40% on managed services, 33% on time-and-materials."),
      soWhat: T("En régie, un dépassement se facture ; au forfait, il est offert au client. Suivre l'avancement en continu permet de négocier un avenant ou de réduire l'équipe pendant la mission, pas après.",
                "On time-and-materials an overrun gets invoiced; on fixed-price it is given away to the client. Tracking progress continuously makes it possible to negotiate an amendment or shrink the team during the engagement, not after."),
      driver: T("Chiffrage initial optimiste et surcharge déclarée en fin de mission pour tenir les délais.",
                "Optimistic initial estimates and overtime logged at the end of the engagement to meet deadlines."),
      team: T("Directeurs de mission · Contrôle de gestion", "Engagement managers · Financial control")
    }
  ],

  matrix: [
    { action: T("Réallouer temporairement des consultants Strategy & Ops vers les practices en tension",
                "Temporarily reallocate Strategy & Ops consultants to practices under pressure"),
      priority: "Haute",
      impact: T("Récupère une part des 4 389 jours d'intercontrat, soit jusqu'à 1,51 M€ de coût sec",
                "Recovers part of the 4,389 bench days, up to €1.51M of dead cost"),
      owner: T("Direction · Staffing", "Leadership · Staffing"),
      kpi: T("Taux d'occupation par practice", "Utilization by practice") },
    { action: T("Revoir chaque semaine les missions au forfait passées 85 % d'avancement",
                "Review every week the fixed-price engagements past 85% progress"),
      priority: "Haute",
      impact: T("Déclenche un avenant ou une réduction d'équipe avant le dépassement",
                "Triggers an amendment or a team reduction before the overrun"),
      owner: T("Directeurs de mission", "Engagement managers"),
      kpi: T("Coût du dépassement · nb de missions en dépassement", "Overrun cost · number of overrunning engagements") },
    { action: T("Fixer aux Managers une part d'activité facturable explicite, distincte de l'avant-vente",
                "Give Managers an explicit billable share, separate from pre-sales"),
      priority: "Moyenne",
      impact: T("Ferme l'écart de 20 points sur le grade le plus coûteux", "Closes the 20-point gap on the costliest grade"),
      owner: T("RH · Directeurs de practice", "HR · Practice directors"),
      kpi: T("Occupation vs cible par grade", "Utilization vs target by grade") },
    { action: T("Bloquer la clôture tant que les saisies en attente dépassent un seuil",
                "Block month-end close while pending entries exceed a threshold"),
      priority: "Moyenne",
      impact: T("Fiabilise les chiffres de comité : 1 902 jours encore non validés", "Makes committee figures reliable: 1,902 days still unvalidated"),
      owner: T("Contrôle de gestion", "Financial control"),
      kpi: T("Taux de complétude · saisies non validées", "Completeness rate · unvalidated entries") },
    { action: T("Publier le rapport sur le service Power BI avec sécurité au niveau des lignes par practice",
                "Publish the report on the Power BI service with row-level security by practice"),
      priority: "Basse",
      impact: T("Chaque directeur ne voit que son périmètre, sans copie de fichier", "Each director sees only their scope, with no file copies"),
      owner: T("Équipe BI", "BI team"),
      kpi: T("Nombre d'utilisateurs actifs", "Active users") }
  ],

  appendix: {
    code: [
      {
        label: T("Modèle en étoile", "Star schema"),
        lang: "text",
        body: T(
`                 Dim_Date (marquée table de dates)
                    │1                 ╎ inactives : Date_Debut, Date_Fin_Prevue
                    ▼*                 ╎
Dim_Consultant ─1──► Fact_Temps ◄──1─ Dim_Mission ─*──1─ Dim_Client
     │*   │*              ▲*                │*
     ▼1   ▼1              │1               ▼1 (inactive)
Dim_Grade Dim_Practice  Dim_Activite     Dim_Practice
     │1
     ▼*
Fact_Objectifs            Fact_Facturation ─*──1─ Dim_Mission, Dim_Date

 11 tables · 14 relations, toutes en 1 → *, filtre croisé unique
 Aucune relation * → * : quand deux chemins mènent à la même table,
 un seul reste actif, les autres sont appelés par USERELATIONSHIP.
 Dim_Pont_Marge : table déconnectée qui porte l'ordre du pont de marge.`,
`                 Dim_Date (marked as date table)
                    │1                 ╎ inactive: Date_Debut, Date_Fin_Prevue
                    ▼*                 ╎
Dim_Consultant ─1──► Fact_Temps ◄──1─ Dim_Mission ─*──1─ Dim_Client
     │*   │*              ▲*                │*
     ▼1   ▼1              │1               ▼1 (inactive)
...
Fact_Objectifs            Fact_Facturation ─*──1─ Dim_Mission, Dim_Date
 ...
 11 tables · 14 relationships, all 1 → *, single cross-filter
 No * → * relationship: when two paths lead to the same table, only one
 stays active, the others are invoked through USERELATIONSHIP.
 Dim_Pont_Marge: a disconnected table carrying the margin bridge order.`)
      },
      {
        label: T("DAX — occupation", "DAX — utilization"),
        lang: "text",
        body:
`Jours facturables =
CALCULATE ( [Jours saisis], Dim_Activite[Est_Facturable] = 1 )

Jours disponibles =
CALCULATE ( [Jours saisis], Dim_Activite[Categorie_Activite] <> "Absence" )

Taux d'occupation = DIVIDE ( [Jours facturables], [Jours disponibles] )

-- La cible dépend du grade : moyenne PONDÉRÉE par les jours disponibles
Taux d'occupation cible =
DIVIDE (
    SUMX (
        VALUES ( Dim_Grade[ID_Grade] ),
        [Jours disponibles] * CALCULATE ( MAX ( Fact_Objectifs[Taux_Occupation_Cible] ) )
    ),
    [Jours disponibles]
)`
      },
      {
        label: T("DAX — dépassement", "DAX — overrun"),
        lang: "text",
        body: T(
`-- Écrit d'abord comme [Jours consommés] - [Jours vendus] : juste à la ligne,
-- faux au total (0 €), car les missions sous budget compensaient les autres.
-- SUMX force le calcul mission par mission AVANT de sommer.
Coût du dépassement =
SUMX (
    VALUES ( Dim_Mission[ID_Mission] ),
    VAR Ecart = [Jours consommés] - [Jours vendus]
    RETURN IF ( Ecart > 0, Ecart * [Coût journalier moyen], 0 )
)

-- Rôle de date multiple : la relation Date_Debut reste inactive
Missions démarrées dans la période =
CALCULATE (
    COUNTROWS ( Dim_Mission ),
    USERELATIONSHIP ( Dim_Date[Date], Dim_Mission[Date_Debut] )
)`,
`-- First written as [Jours consommés] - [Jours vendus]: right on each row,
-- wrong on the total (€0), because under-budget engagements offset the others.
-- SUMX forces the calculation engagement by engagement BEFORE summing.
Coût du dépassement =
SUMX (
    VALUES ( Dim_Mission[ID_Mission] ),
    VAR Ecart = [Jours consommés] - [Jours vendus]
...
-- Role-playing date: the Date_Debut relationship stays inactive
Missions démarrées dans la période =
CALCULATE (
    COUNTROWS ( Dim_Mission ),
    USERELATIONSHIP ( Dim_Date[Date], Dim_Mission[Date_Debut] )
      ...
      {
        label: T("DAX — pont de marge", "DAX — margin bridge"),
        lang: "text",
        body: T(
`-- Table déconnectée : une ligne par barre du graphique en cascade
Dim_Pont_Marge =
DATATABLE ( "Categorie", STRING, "Ordre", INTEGER, {
    { "CA facturé", 1 }, { "Production", 2 }, { "Intercontrat", 3 },
    { "Avant-vente", 4 }, { "Interne, congés, R&D", 5 } } )

-- Valeur signée par barre ; la marge est le TOTAL natif du visuel,
-- pas une ligne de plus (elle serait ajoutée au lieu d'être posée)
Valeur Pont Marge =
SWITCH ( SELECTEDVALUE ( Dim_Pont_Marge[Categorie] ),
    "CA facturé",   [CA facturé],
    "Production",   - CALCULATE ( [Coût interne], Dim_Activite[ID_Activite] = "A01" ),
    "Intercontrat", - CALCULATE ( [Coût interne], Dim_Activite[ID_Activite] = "A03" ),
    "Avant-vente",  - CALCULATE ( [Coût interne], Dim_Activite[ID_Activite] = "A02" ),
    "Interne, congés, R&D",
        - CALCULATE ( [Coût interne], Dim_Activite[ID_Activite] IN { "A04","A05","A06","A07","A08" } ) )`,
`-- Disconnected table: one row per bar of the waterfall chart
Dim_Pont_Marge =
DATATABLE ( "Categorie", STRING, "Ordre", INTEGER, {
    { "CA facturé", 1 }, { "Production", 2 }, { "Intercontrat", 3 },
    { "Avant-vente", 4 }, { "Interne, congés, R&D", 5 } } )

-- Signed value per bar; the margin is the visual's native TOTAL,
-- not one more row (it would be added instead of set)
Valeur Pont Marge =
SWITCH ( SELECTEDVALUE ( Dim_Pont_Marge[Categorie] ),
    "CA facturé",   [CA facturé],
    "Production",   - CALCULATE ( [Coût interne], Dim_Activite[ID_Activite] = "A01" ),
    ...
    difficulties: [
      { p: T("Des colonnes entières en erreur au premier changement de type : Power BI en locale française lisait « 415.9 » comme un texte invalide.",
             "Whole columns in error on the first type change: Power BI in a French locale read “415.9” as invalid text."),
        s: T("Conversion « avec paramètres régionaux » en anglais (États-Unis) sur chaque colonne décimale, et jamais un simple remplacement de texte.",
             "A “using locale” conversion set to English (United States) on every decimal column, never a plain text replace.") },
      { p: T("La relation Practice → Mission s'est désactivée toute seule : Practice atteignait Fact_Temps par deux chemins, via le consultant et via la mission.",
             "The Practice → Engagement relationship switched itself off: Practice reached Fact_Temps through two paths, via the consultant and via the engagement."),
        s: T("Garder actif le chemin par le consultant — l'occupation se mesure sur la practice d'origine du consultant — et appeler l'autre par USERELATIONSHIP là où il sert.",
             "Keep the consultant path active — utilization is measured on the consultant's home practice — and invoke the other through USERELATIONSHIP where it is needed.") },
      { p: T("Relier les objectifs au calendrier imposait une relation plusieurs-à-plusieurs : le calendrier est au jour, les objectifs au mois × grade.",
             "Linking targets to the calendar forced a many-to-many relationship: the calendar is daily, targets are month × grade."),
        s: T("Pas de relation forcée : la cible étant constante par grade, Fact_Objectifs se relie au seul grade, et la mesure pondère par les jours disponibles.",
             "No forced relationship: since the target is constant per grade, Fact_Objectifs links to the grade alone, and the measure weights by available days.") },
      { p: T("Le total du coût de dépassement affichait 0 € alors que chaque ligne était juste.",
             "The overrun cost total showed €0 while every row was right."),
        s: T("Le total recalcule la mesure sur l'ensemble : la différence des totaux compensait les missions sous budget. Réécriture en SUMX mission par mission.",
             "The total re-evaluates the measure over everything: the difference of totals offset under-budget engagements. Rewritten as a per-engagement SUMX.") },
      { p: T("Les colonnes de la matrice mensuelle affichaient des dates complètes : une hiérarchie de dates automatique fantôme restait accrochée à la colonne mois.",
             "The monthly matrix columns showed full dates: a leftover automatic date hierarchy stayed attached to the month column."),
        s: T("Une colonne neuve dérivée directement de la date — FORMAT ( Dim_Date[Date], \"YYYY-MM\" ) — triée par une clé numérique calculée elle aussi à partir de la date.",
             "A new column derived straight from the date — FORMAT ( Dim_Date[Date], \"YYYY-MM\" ) — sorted by a numeric key also computed from the date.") },
      { p: T("Power BI n'a ni bascule de langue ni bascule clair / sombre.",
             "Power BI has neither a language toggle nor a light / dark toggle."),
        s: T("Signets et groupes de visuels superposés, une version par langue et par thème : quatre signets par page, déclenchés par les boutons FR / EN et lune / soleil.",
             "Bookmarks and stacked visual groups, one version per language and per theme: four bookmarks per page, triggered by the FR / EN and moon / sun buttons.") }
    ],
    limits: [
      T("Données synthétiques : le générateur reproduit des régularités réalistes, il ne remplace pas un historique réel de feuilles de temps.",
        "Synthetic data: the generator reproduces realistic regularities, it does not replace a real timesheet history."),
      T("La publication web publique dépend d'un paramètre d'administration du tenant Power BI ; le rapport se consulte ici en captures.",
        "Public web publishing depends on a Power BI tenant admin setting; the report is shown here as screenshots."),
      T("Le mode clair / sombre bascule les fonds et les textes principaux, pas le formatage de chaque graphique.",
        "The light / dark mode switches backgrounds and main text, not the formatting of every chart."),
      T("Aucune cible de marge n'existe dans les données : elle n'a pas été inventée, la carte de marge ne se compare donc à rien.",
        "No margin target exists in the data: it was not invented, so the margin card is compared with nothing."),
      T("Pas de sécurité au niveau des lignes : tout lecteur voit toutes les practices.",
        "No row-level security: every reader sees every practice.")
    ]
  },

  run: "Power BI Desktop   →   consulting dashboard.pbip"
},

/* ═════════════════════════════════════════════════════════════════════════
   02 — INVISIBLE CHURN DETECTOR
   ═════════════════════════════════════════════════════════════════════════ */
{
...
/* ═════════════════════════════════════════════════════════════════════════
   03 — REVENUE DROP DIAGNOSTIC ENGINE
   ═════════════════════════════════════════════════════════════════════════ */
{
  slug: "revenue-drop",
  num: "03",
...
/* ═════════════════════════════════════════════════════════════════════════
   04 — INVENTORY LOST SALES DETECTOR
   ═════════════════════════════════════════════════════════════════════════ */
{
  slug: "inventory-lost",
  num: "04",
...
/* ═════════════════════════════════════════════════════════════════════════
   05 — MARKETING CONTACT FATIGUE OPTIMIZER
   ═════════════════════════════════════════════════════════════════════════ */
{
...
/* ═════════════════════════════════════════════════════════════════════════
   06 — CUSTOMER SUPPORT ESCALATION PREDICTOR
   ═════════════════════════════════════════════════════════════════════════ */
{
